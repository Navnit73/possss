        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 border border-zinc-200 bg-white rounded-lg shadow-sm overflow-hidden">
            ...
            <div className="p-6 border-b md:border-b-0 md:border-r border-zinc-200">
              ...
              <h3 className="text-3xl font-mono text-amber-600 font-bold">{formatCurrency(data.metrics.purchase_value)}</h3>
            </div>
            <div className="p-6">
              <p className="text-xs font-semibold text-zinc-500 uppercase tracking-widest mb-2">Expected Profit</p>
              <h3 className="text-3xl font-mono text-emerald-600 font-bold">{formatCurrency(data.metrics.expected_profit)}</h3>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-white border border-zinc-200 rounded-lg shadow-sm p-6">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xs font-bold text-zinc-400 uppercase tracking-widest">2. Cost Value vs Expected Retail Revenue ({currencySymbol})</h3>
                  ...
                  {data.table.data.length === 0 ? (
                    ...
                    data.table.data.map((row: any) => (
                      <tr key={row._id} className="hover:bg-zinc-50 transition-colors">
                        ...
                        {visibleColumns.selling_price && <td className="px-6 py-4 text-right font-mono text-zinc-900">{formatCurrency(row.selling_price)}</td>}
                        {visibleColumns.total_cost_value && <td className="px-6 py-4 text-right font-mono font-bold text-amber-600">{formatCurrency(row.total_cost_value)}</td>}
